...
export function Relais({
  ...
  const switchOn: MouseEventHandler<HTMLDivElement> = (event) => {
    ...
    if (!isLoggedIn())
      setBusy(true)
    try {
      if (state.id === sysInfo.chlorineDosageRelais) {
        controllerService.manualChlorineDosage()
      } else if (state.id === sysInfo.phMinusDosageRelais) {
        controllerService.manualPhMinusDosage()
      } else if (state.id === sysInfo.phPlusDosageRelais) {
        controllerService.manualPhPlusDosage()
      } else {
  ...
  // count down to zero
  const [countdown, setCountdown] = useState(dosage ? dosage.next_cycle : 0)
  useEffect(() => {
